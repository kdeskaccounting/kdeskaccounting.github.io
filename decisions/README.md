...
- **Only a LATER entry answers**, with one deliberate asymmetry. An **approval** must come from a **strictly greater** id: an entry cannot approve **itself** — which is what a T2 row asking for its own veto window would be doing — and an old entry cannot pre-authorise a decision that had not been made yet. A **veto** counts from the answered row onwards (id **≥** the id it answers), so a row that vetoes itself closes its own gate, exactly as `"status": "vetoed"` does: a stop is never ignored on a technicality. (Ids are allocated in file order here, so id order *is* file order; `ts` is not consulted — it is a string a hand-written row can set to anything.)
