...
$V scripts/browser/session.py --check youtube                      # is Studio still signed in?
...
# --go is REQUIRED for a real post (immediate public video). publish.py --dry-run REFUSES this
# platform — its own contract is that a dry run writes nothing, and this one cannot honour it.
$V scripts/publishers/youtube_web.py --go --asset <mp4> --meta <json>                 # the real post
```
